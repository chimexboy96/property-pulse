...
const ProfilePage = () => {
    ...
    useEffect(() => {
        ...
        // fetch user property when session is available 
        if (session?.user?.id) {
    ...
    const handleDeleteProperty= async (propertyId) => {
        ...
        try {
            ...
            if (res.status === 200) {
                // Remove the propery from the state
                const updatedProproperties= properties.filter((property) => property._id !== propertyId )
                setProperties(updatedProproperties)
